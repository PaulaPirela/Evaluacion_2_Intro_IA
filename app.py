...
:root {
    ...
    --secondary-bg: #2b2b2b;    /* Gris para respuestas del agente */
    --user-message-bg: #3c4043;  /* Gris un poco más claro para mensajes de usuario */
...
/* <<--- ESTILOS CORREGIDOS Y DEFINITIVOS PARA LAS BURBUJAS DE CHAT --->> */
[data-testid="stChatMessage"] > div[data-testid="stMarkdown"] {
    padding: 1.2em; border-radius: 12px; line-height: 1.6;
    color: var(--text-light) !important; /* Texto blanco para TODOS los mensajes */
}
...
/* Mensajes del Usuario: Fondo oscuro, texto blanco */
...
# --- INTERFAZ DE CHAT PRINCIPAL ---
    ...
    if prompt:
        if st.session_state.selected_prompt: st.session_state.selected_prompt = None
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user", avatar=USER_AVATAR):
            st.markdown(prompt)
        with st.chat_message("assistant", avatar=BOT_AVATAR):
            try:
                response = st.write_stream(st.session_state.chain.stream({"input": prompt}))
